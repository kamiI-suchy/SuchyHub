					 ...
					 const struct blk_mq_queue_data *bd)
{
	...
	rq_for_each_segment(bvec, req, iter) {
		void *page_mem = kmap_local_page(bvec.bv_page);
...
static int __init index_block_init(void)
{
	...
	ib_dev.gd = blk_mq_alloc_disk(&ib_dev.tag_set, &ib_dev);
	if (IS_ERR(ib_dev.gd)) {
		ret = PTR_ERR(ib_dev.gd);
		goto err_tag_set;
	}

	ib_dev.queue = ib_dev.gd->queue;
...
MODULE_DESCRIPTION("Sterownik blokowy zwracający numer indeksu po poprawnym zapisie");
