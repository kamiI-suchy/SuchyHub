...
static struct delayed_work work_yellow;
static struct delayed_work work_blue;
...
{
	schedule_delayed_work(&work_yellow, msecs_to_jiffies(DELAY_MS));
	return IRQ_HANDLED;
}

static irqreturn_t button_blue_isr(int irq, void *dev_id)
{
	schedule_delayed_work(&work_blue, msecs_to_jiffies(DELAY_MS));
	return IRQ_HANDLED;
}
...
static int __init interrupt_init(void)
{
	...
	INIT_DELAYED_WORK(&work_yellow, button_yellow_work);
	INIT_DELAYED_WORK(&work_blue, button_blue_work);
...
{
	cancel_delayed_work_sync(&work_yellow);
	cancel_delayed_work_sync(&work_blue);
	free_irq(irq_yellow, NULL);
	free_irq(irq_blue, NULL);
	gpio_free(BUTTON_YELLOW);
	gpio_free(BUTTON_BLUE);
