...
MODULE_DESCRIPTION("Sterownik przerwań dla przycisków GPIO68 (żółty) i GPIO69 (niebieski)");
...
{
	printk(KERN_INFO "Wciśnięty przycisk GPIO68 (żółty)\n");
	return IRQ_HANDLED;
}

static irqreturn_t button_blue_isr(int irq, void *dev_id)
{
	printk(KERN_INFO "Wciśnięty przycisk GPIO69 (niebieski)\n");
	return IRQ_HANDLED;
}
...
static int __init interrupt_init(void)
{
	...
	printk(KERN_INFO "Ładowanie modułu przerwań...\n");
	...
	printk(KERN_INFO "Moduł przerwań załadowany (GPIO68 IRQ=%d, GPIO69 IRQ=%d)\n",
	       irq_yellow, irq_blue);
	return 0;
...
static void __exit interrupt_exit(void)
{
	...
	gpio_free(BUTTON_YELLOW);
	gpio_free(BUTTON_BLUE);
	printk(KERN_INFO "Moduł przerwań wyładowany\n");
}
