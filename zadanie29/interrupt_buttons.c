...
MODULE_DESCRIPTION("Sterownik przerwan dla przyciskow GPIO68 (zolty) i GPIO69 (niebieski) z workqueue");
...
{
	printk(KERN_INFO "Kamil Suchy 60859 - obsluga przycisku GPIO68 (zolty)\n");
}

static void button_blue_work(struct work_struct *work)
{
	printk(KERN_INFO "Kamil Suchy 60859 - obsluga przycisku GPIO69 (niebieski)\n");
}
...
static int __init interrupt_init(void)
{
	...
	printk(KERN_INFO "Ladowanie modulu przerwan z workqueue...\n");
	...
	printk(KERN_INFO "Modul przerwan z workqueue zaladowany (GPIO68 IRQ=%d, GPIO69 IRQ=%d)\n",
	       irq_yellow, irq_blue);
	return 0;
...
static void __exit interrupt_exit(void)
{
	...
	gpio_free(BUTTON_YELLOW);
	gpio_free(BUTTON_BLUE);
	printk(KERN_INFO "Modul przerwan z workqueue wyladowany\n");
}
